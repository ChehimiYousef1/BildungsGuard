...
// Find insertion point before last </div> before );
const insertBefore = "    </div>\n  );\n}";
const idx = c.lastIndexOf(insertBefore);
console.log('Insert point at:', idx);
...
if (idx > -1) {
  c = c.slice(0, idx) + modal + insertBefore + c.slice(idx + insertBefore.length);
