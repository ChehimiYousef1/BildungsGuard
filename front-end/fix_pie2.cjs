...
const startIdx = c.lastIndexOf(startMarker);
...
const newSection = `      {widgets.compliance && (
        <div className="card" style={{ padding: '19px 8px 12px' }}>
          ...
          <div style={{ display: 'flex', gap: 14, padding: '10px 14px 0', flexWrap: 'wrap' }}>
            <span style={{ fontSize: 11, color: '#085041', display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 10, height: 10, borderRadius: '50%', background: '#1D9E75', display: 'inline-block' }}/>{lang === 'de' ? 'Konform' : 'Compliant'}</span>
            <span style={{ fontSize: 11, color: '#633806', display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 10, height: 10, borderRadius: '50%', background: '#BA7517', display: 'inline-block' }}/>{lang === 'de' ? 'Prüfen' : 'Review'}</span>
            <span style={{ fontSize: 11, color: '#501313', display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ width: 10, height: 10, borderRadius: '50%', background: '#A32D2D', display: 'inline-block' }}/>{lang === 'de' ? 'Kritisch' : 'Critical'}</span>
          </div>
        </div>
      )}`;
