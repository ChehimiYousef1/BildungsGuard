...
export default function QM() {
  ...
  return (
    <>
      ...
      {tab === 'audit' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="card" style={{ padding: '19px 8px 8px' }}>
            <div className="card-head" style={{ padding: '0 13px 14px' }}>
              <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <BadgeCheck size={15} color={C.iris} />
                {de ? 'Interne Audits' : 'Internal audits'} · {INT_AUDITS.length}
              </div>
              <ExcelBtn onClick={exportAudits} />
            </div>
            <div className="scroll-x">
              <table>
                <thead><tr><th>{de ? 'Datum' : 'Date'}</th><th>{de ? 'Art' : 'Type'}</th><th className="hide-mobile">{de ? 'Befunde' : 'Findings'}</th><th className="hide-mobile">{de ? 'Offen' : 'Open'}</th><th>Status</th></tr></thead>
                <tbody>
                  {INT_AUDITS.map((a, i) => {
                    const color = a.status === 'completed' ? C.mint : a.status === 'inProgress' ? C.amber : C.muted;
                    return (
                      <tr key={i} className="row">
                        <td style={{ fontSize: 12 }}>{a.date}</td>
                        <td style={{ fontWeight: 500, fontSize: 13 }}>{de ? a.tde : a.ten}</td>
                        <td className="hide-mobile" style={{ fontSize: 12, color: C.muted }}>{a.findings}</td>
                        <td className="hide-mobile" style={{ fontSize: 12, color: C.muted }}>{a.open}</td>
                        <td><span className="badge" style={{ background: color + '18', color, fontSize: 11 }}>{a.status === 'completed' ? (de ? 'Abgeschlossen' : 'Completed') : a.status === 'inProgress' ? (de ? 'Laufend' : 'In progress') : (de ? 'Geplant' : 'Planned')}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="card" style={{ padding: '19px 8px 8px' }}>
            <div className="card-head" style={{ padding: '0 13px 14px' }}>
              <div className="card-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {de ? 'Externe Audits (AZAV)' : 'External audits (AZAV)'} · {AUDIT_HIST.length}
              </div>
            </div>
            <div className="scroll-x">
              <table>
                <thead><tr><th>{de ? 'Datum' : 'Date'}</th><th>{de ? 'Stelle' : 'Body'}</th><th>{de ? 'Art' : 'Type'}</th><th className="hide-mobile">{de ? 'Befunde' : 'Findings'}</th><th>Status</th></tr></thead>
                <tbody>
                  {AUDIT_HIST.map((a, i) => {
                    const color = a.status === 'passed' || a.status === 'fixed' ? C.mint : C.amber;
                    return (
                      <tr key={i} className="row">
                        <td style={{ fontSize: 12 }}>{a.date}</td>
                        <td style={{ fontSize: 12.5 }}>{a.stelle}</td>
                        <td style={{ fontWeight: 500, fontSize: 13 }}>{de ? a.tde : a.ten}</td>
                        <td className="hide-mobile" style={{ fontSize: 12, color: C.muted }}>{de ? a.fde : a.fen}</td>
                        <td><span className="badge" style={{ background: color + '18', color, fontSize: 11 }}>{a.status === 'fixed' ? (de ? 'Behoben' : 'Fixed') : a.status === 'passed' ? (de ? 'Bestanden' : 'Passed') : a.status}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {/* ===== VIEW MODAL ===== */}
      {viewDoc && (
        <div onClick={() => setViewDoc(null)} style={overlay}>
          <div onClick={(e) => e.stopPropagation()} className="card"
            style={{ width: '100%', maxWidth: 460, padding: 22, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div className="card-title" style={{ fontSize: 16 }}>{de ? (viewDoc.titleDe ?? viewDoc.title) : (viewDoc.title ?? viewDoc.titleDe)}</div>
              <button onClick={() => setViewDoc(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted }}><X size={18} /></button>
            </div>
            {(() => {
              const isValid  = viewDoc.status === 'valid' || viewDoc.status === 'doc_ready';
              const isReview = viewDoc.status === 'inReview';
              const color    = isValid ? C.mint : isReview ? C.amber : C.rose;
              return (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderRadius: 12, background: color + '0D', marginBottom: 14 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 11, background: color, display: 'grid', placeItems: 'center' }}><FileText size={22} color="#fff" /></div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 15 }}>{de ? (viewDoc.titleDe ?? viewDoc.title) : (viewDoc.title ?? viewDoc.titleDe)}</div>
                    <div style={{ fontSize: 12, color: C.muted }}>{typeLabel(viewDoc.type)} · {viewDoc.version ?? '—'}</div>
                  </div>
                  <span className="badge" style={{ background: color + '18', color }}>
                    {isValid ? (de ? 'Gültig ✅' : 'Valid ✅') : isReview ? (de ? 'In Prüfung ⚠️' : 'In review ⚠️') : (de ? 'Entwurf' : 'Draft')}
                  </span>
                </div>
              );
            })()}
            {[[de ? 'Typ' : 'Type', typeLabel(viewDoc.type)], ['Version', viewDoc.version ?? '—'], [de ? 'Verantwortlich' : 'Author', viewDoc.owner ?? viewDoc.author ?? '—'], [de ? 'Genehmigt am' : 'Approved', viewDoc.approved ?? '—']].map(([label, value], i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${C.lineSoft}`, fontSize: 13 }}>
            ...
            {viewDoc.content && (
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 11, color: C.muted, marginBottom: 6 }}>{de ? 'Inhalt' : 'Content'}</div>
                <div style={{ fontSize: 13, color: C.inkSoft, lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{viewDoc.content}</div>
              </div>
            )}
            <div style={{ display: 'flex', gap: 8, marginTop: 18 }}>
              <button className="btn btn-primary" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={() => downloadQmDoc(viewDoc)}>
                <Download size={14} /> {de ? 'Herunterladen' : 'Download'}
              </button>
              <button className="btn btn-ghost" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} disabled={uploading === viewDoc.id} onClick={() => fileUploadRef.current[viewDoc.id]?.click()}>
                <Upload size={14} /> {uploading === viewDoc.id ? '…' : (de ? 'Hochladen' : 'Upload')}
              </button>
              <button className="btn btn-ghost" style={{ padding: '9px 14px' }} onClick={() => { setViewDoc(null); openEditDoc(viewDoc); }}>
                <Pencil size={14} />
              </button>
            </div>
          </div>
      ...
      {open && (
        <div onClick={() => !saving && setOpen(false)} style={overlay}>
          <div onClick={(e) => e.stopPropagation()} className="card" style={{ width: '100%', maxWidth: 520, padding: 22, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div className="card-title" style={{ fontSize: 16 }}>{editId ? (de ? 'Dokument bearbeiten' : 'Edit document') : (de ? 'Neues QM-Dokument' : 'New QM document')}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <label style={lbl}>{de ? 'Typ' : 'Type'}
                <select value={form.type} onChange={(e) => set('type', e.target.value)} style={inp}>
                  <option value="handbook">{de ? 'Handbuch' : 'Handbook'}</option>
                  <option value="process">{de ? 'Prozess' : 'Process'}</option>
                  <option value="form">{de ? 'Formular' : 'Form'}</option>
                </select>
              </label>
              <label style={lbl}>{de ? 'Titel *' : 'Title *'}<input value={form.title} onChange={(e) => set('title', e.target.value)} style={inp} placeholder={de ? 'z.B. Anwesenheitskontrolle' : 'e.g. Attendance check'} /></label>
              <label style={lbl}>{de ? 'Inhalt' : 'Content'}<textarea value={form.content} onChange={(e) => set('content', e.target.value)} style={{ ...inp, minHeight: 100, resize: 'vertical' }} placeholder={de ? 'Beschreibung…' : 'Description…'} /></label>
              <div style={{ display: 'flex', gap: 10 }}>
                <label style={{ ...lbl, flex: 1 }}>{de ? 'Verantwortlich' : 'Owner'}<input value={form.owner} onChange={(e) => set('owner', e.target.value)} style={inp} /></label>
                <label style={{ ...lbl, flex: 1 }}>Version<input value={form.version} onChange={(e) => set('version', e.target.value)} style={inp} placeholder="v1.0" /></label>
              </div>
              <label style={lbl}>Status
                <select value={form.status} onChange={(e) => set('status', e.target.value)} style={inp}>
                  <option value="doc_ready">{de ? 'Gültig' : 'Valid'}</option>
                  <option value="inReview">{de ? 'In Prüfung' : 'In review'}</option>
                  <option value="doc_missing">{de ? 'Entwurf' : 'Draft'}</option>
                </select>
              </label>
      ...
      {/* ===== CAPA FORM MODAL ===== */}
      {capaOpen && (
        <div onClick={() => !capaSaving && setCapaOpen(false)} style={overlay}>
          <div onClick={(e) => e.stopPropagation()} className="card" style={{ width: '100%', maxWidth: 520, padding: 22, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div className="card-title" style={{ fontSize: 16 }}>{capaEditId ? (de ? 'Fall bearbeiten' : 'Edit case') : (de ? 'Neuer Fall' : 'New case')}</div>
              <button onClick={() => !capaSaving && setCapaOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted }}><X size={18} /></button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', gap: 10 }}>
                <label style={{ ...lbl, flex: 1 }}>{de ? 'Datum' : 'Date'}<input value={capaForm.date} onChange={(e) => capaSet('date', e.target.value)} style={inp} placeholder="TT.MM.JJJJ" /></label>
                <label style={{ ...lbl, flex: 1 }}>{de ? 'Quelle' : 'Source'}
                  <select value={capaForm.source} onChange={(e) => capaSet('source', e.target.value)} style={inp}>
                    <option value="Complaint">{de ? 'Beschwerde' : 'Complaint'}</option>
                    <option value="Audit">Audit</option>
                    <option value="Observation">{de ? 'Beobachtung' : 'Observation'}</option>
                    <option value="Feedback">Feedback</option>
                  </select>
                </label>
              </div>
              <label style={lbl}>{de ? 'Beschreibung *' : 'Description *'}<textarea value={capaForm.description} onChange={(e) => capaSet('description', e.target.value)} style={{ ...inp, minHeight: 80, resize: 'vertical' }} placeholder={de ? 'Was ist passiert?' : 'What happened?'} /></label>
              <div style={{ display: 'flex', gap: 10 }}>
                <label style={{ ...lbl, flex: 1 }}>{de ? 'Kategorie' : 'Category'}<input value={capaForm.category} onChange={(e) => capaSet('category', e.target.value)} style={inp} placeholder={de ? 'z.B. Organisation' : 'e.g. Organization'} /></label>
                <label style={{ ...lbl, flex: 1 }}>{de ? 'Verantwortlich' : 'Owner'}<input value={capaForm.owner} onChange={(e) => capaSet('owner', e.target.value)} style={inp} /></label>
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <label style={{ ...lbl, flex: 1 }}>{de ? 'Fällig am' : 'Due date'}<input value={capaForm.dueDate} onChange={(e) => capaSet('dueDate', e.target.value)} style={inp} placeholder="TT.MM.JJJJ" /></label>
                <label style={{ ...lbl, flex: 1 }}>Status
                  <select value={capaForm.status} onChange={(e) => capaSet('status', e.target.value)} style={inp}>
                    <option value="open">{de ? 'Offen' : 'Open'}</option>
                    <option value="in_progress">{de ? 'In Bearbeitung' : 'In progress'}</option>
                    <option value="closed">{de ? 'Geschlossen' : 'Closed'}</option>
                    <option value="overdue">{de ? 'Überfällig' : 'Overdue'}</option>
                  </select>
                </label>
              </div>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
                <button className="btn" style={{ padding: '9px 16px', background: C.soft, color: C.inkSoft }} disabled={capaSaving} onClick={() => setCapaOpen(false)}>{de ? 'Abbrechen' : 'Cancel'}</button>
                <button className="btn btn-primary" style={{ padding: '9px 16px' }} disabled={capaSaving} onClick={submitCapa}>{capaSaving ? '…' : (de ? 'Speichern' : 'Save')}</button>
              </div>
            </div>
          </div>
        </div>
